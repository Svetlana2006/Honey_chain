...
router.post("/", upload.single("photo"), async (req, res) => {
  try {
    ...
    const verifyUrl = `http://localhost:5173/verify?id=${batch.id}`;
...
router.get("/:id", (req, res) => {
  const scan = db.recordBatchScan(req.params.id);
  if (!scan) return res.status(404).json({ error: "Batch not found" });
  ...
  res.json({
    ...
    block,
    scan: {
      firstScan: !scan.alreadyVerified,
      alreadyVerified: scan.alreadyVerified,
      verifiedAt: scan.batch.verifiedAt,
    },
  });
});
...
router.post("/:id/tamper", (req, res) => {
  ...
  if (!tamperable.includes(field)) {
    return res
      .status(400)
      .json({ error: `Field "${field}" is not a chain-committed field` });
  }
